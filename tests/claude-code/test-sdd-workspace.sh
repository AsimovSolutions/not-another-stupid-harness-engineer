...
PLAN
    ...
    if [[ "$dir_a" == "$repo/.nashe/sdd/plan-a" ]]; then
        pass "prints <repo-root>/.nashe/sdd/<plan-basename>"
    else
        fail "prints <repo-root>/.nashe/sdd/<plan-basename>"
        echo "    got: $dir_a"
    fi
    ...
    if [[ -f "$repo/.nashe/sdd/.gitignore" && "$(cat "$repo/.nashe/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created at .nashe/sdd/ with '*'"
    else
        fail "self-ignoring .gitignore created at .nashe/sdd/ with '*'"
    fi
    ...
    if [[ "$brief_path" == "$repo/.nashe/sdd/plan-a/task-1-brief.md" ]]; then
    ...
    case "$rp_path" in
        "$repo/.nashe/sdd/plan-a/review-"*.diff)
    ...
    if [[ "$wt_dir" == "$wt_root/.nashe/sdd/plan-a" && "$wt_dir" != "$dir_a" ]]; then
